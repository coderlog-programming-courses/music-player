...
def play_music(id): # Створення функціїї play_music яка приймає єдиний аргумент,id(айди аудіофайлу з бази даних sqlite3 таблиці musics)

        mixer.init() # Використання модуля mixer з бібліотеки pygame для створення репозиторію для аудіофайлу

        mixer.music.load("{}".format(id)) # Використання модуля mixer з бібліотеки pygame для загрузки обраного аудіофайлу до вже створеного вище репозиторію
