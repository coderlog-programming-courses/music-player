...
def pause_music(): # Створення функціїї pause_music
        ...
        try:
                mixer.music.pause() # Використання модуля mixer з бібліотеки pygame для паузи запущеного аудіофайлу
...
def play_music(way): # Створення функціїї play_music яка приймає єдиний аргумент,id(айди аудіофайлу з бази даних sqlite3 таблиці musics)
        ...
        try: 
                mixer.music.play()# Використання модуля mixer з бібліотеки pygame для запуску обраного аудіо файлуфайлу
...
def rewind_music(second): # Створення функції rewind_music яка приймає єдиний аргумент,second(час на який' потрібно перемотати потрібно перемотати,записується в секундах)
        ...
        try:
                mixer.music.set_pos(int("{}".format(second))) # Використання модуля mixer з бібліотеки pygame для перемотки обраного аудіофайлу на обрани час(секунду)
...
def unpause_music(): # Створення функціїї unpause_music
        ...
        try:
                mixer.music.unpause() # Використання модуля mixer з бібліотеки pygame для відтворення аудіофайлу який був поствлений на паузу за допомогою функції pause_music
...
def stop_music(): # Створення функції stop_music
        ...
        try:
                mixer.music.stop() # Використання модуля mixer з бібліотеки pygame для повного зупинення аудіофайлу тобто для вимкнення вибраного треку
